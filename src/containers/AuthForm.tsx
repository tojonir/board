...
const AuthForm: FC<AuthFormProps> = ({ isSignUp }) => {
  return (
    <div className="w-2/3 h-2/3 max-w-[380px] bg-white rounded-[3px] p-4 flex flex-col justify-between">
      <div>
        <div className="flex flex-col">
          {isSignUp && <Input label="mail" leftIcon={solid("envelope")} />}
        ...
        <div className="p-2 grid grid-cols-1 gap-4 text-gray-600">
          <div className="border rounded-[3px] p-2">
            ...
            <span className="ml-2">Log with Github</span>
          </div>
          <div className="border rounded-[3px] p-2">
            <FontAwesomeIcon icon={brands("google")} />
            <span className="ml-2">Log with Google</span>
          </div>
        </div>
      </div>
